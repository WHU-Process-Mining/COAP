...
for event_log in event_logs:
    log_name = f"prodrift_{event_log}_5k"
    if not path.isfile(path.join("eventlog", "CSV", log_name + ".csv")):
        generate_csv(log_name)
    print(f"Processing event log: {log_name}")
    ...
    for discovery_algorithm, prediction_algorithm, dynamic_update, update_strategy, apply_constraint, use_consistency, use_only_conflict_data, confidence_threshold, consistency_alpha, win_min, drift_threshold in product(
        ...
        if dynamic_update and use_consistency and apply_constraint:
            ...
            if accuracy > best_accuracy:
                ...
                with open(f"experiments/results_{log_name}.pickle", "wb") as file:
                    pickle.dump(results, file)
        else:
            if apply_constraint:
                with open(f"experiments/results_{log_name}_no_consistency.pickle", "wb") as file:
                    pickle.dump(results, file)
            else:
                with open(f"experiments/results_{log_name}_no_constraint.pickle", "wb") as file:
                    pickle.dump(results, file)

        if not dynamic_update:
            with open(f"experiments/results_{log_name}_static.pickle", "wb") as file:
                pickle.dump(results, file)